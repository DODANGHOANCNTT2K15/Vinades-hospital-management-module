...
// Lấy keyword tìm kiếm từ GET
$keyword = $nv_Request->get_string('keyword', 'get', '');
$where = '';
if (!empty($keyword)) {
    // Tránh SQL Injection, dùng prepare hoặc quote
    $keyword_esc = $db->quote('%' . $keyword . '%');
    $where = " WHERE hoten LIKE $keyword_esc ";
}
...
$xtpl->assign('KEYWORD', htmlspecialchars($keyword, ENT_QUOTES)); // gán biến keyword cho template
$xtpl->assign('MODULE_NAME', $module_name);

// Tổng số bản ghi có điều kiện tìm kiếm
$total = $db->query("SELECT COUNT(*) FROM " . $tbPatient . $where)->fetchColumn();

$sql = "SELECT * FROM " . $tbPatient . $where . " ORDER BY id ASC
